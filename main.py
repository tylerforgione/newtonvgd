...
def run_softmax(x_train, y_train, x_val, y_val, x_test, y_test, method='gd', epochs=1000, lr=0.0001, batch_size=None):
    softmax = SoftmaxRegression()
    print("Training softmax regression using", method)
    start = time.time()
    metrics = softmax.fit(x_train, y_train, x_val, y_val, method, epochs, lr, batch_size)
    end = time.time()
    print("Training time: ", end - start)
...
def main():
    ...
    best_params = None
    best_model = None
    for epochs, lr, batch_size in product(epochs_range, lr_range, batch_size_range):
        print("\nnumber of epochs:",epochs, ", learning rate:", lr, ", batch size:", batch_size)
        model, _, val_acc = run_softmax(x_train, y_train, x_val, y_val, x_val, y_val, method='gd', epochs=epochs, lr=lr, batch_size=batch_size)
        if val_acc > best_val_acc:
            best_val_acc = val_acc
            best_params = {'epochs': epochs, 'lr': lr, 'batch_size': batch_size}
            best_model = model
    ...
    for epochs in range(1, 10):
        ...
        model, _, val_acc = run_softmax(x_train, y_train, x_val, y_val, x_val, y_val, method='cg', epochs=epochs)
        if val_acc > best_val_acc:
            best_val_acc = val_acc
            best_epochs = epochs
            best_model = model
